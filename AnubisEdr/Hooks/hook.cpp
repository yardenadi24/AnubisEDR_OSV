...
FARPROC GetMessageBeepAddress() {
	...
	pMessageBeep = (messageBeep)GetProcAddress(hUser32, "MessageBeep");
	if (!pMessageBeep) {
		FreeLibrary(hUser32);
		return nullptr;
	}
	return (FARPROC)pMessageBeep;
}
...
FARPROC GetMessageBoxAAddress() {
	...
	pMessageBoxA = (messageBoxA)GetProcAddress(hUser32, "MessageBoxA");
	if (!pMessageBoxA) {
		FreeLibrary(hUser32);
		return nullptr;
	}
	return(FARPROC)pMessageBoxA;
}
